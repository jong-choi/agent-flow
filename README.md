...
- 모델 관리 워커를 활성화하면 Google·Groq·Ollama의 공식 모델 목록을 매주 수집하고, 기존 모델의 제공 여부를 매일 확인하여 무료 사용 검증 대상 후보를 갱신합니다.
- 호출 오류에 따른 일시 사용 중지·복구와 근거에 따른 모델 은퇴를 관리하며, 기존 워크플로우의 모델 참조를 보존하고 대체 모델 선택을 지원합니다.
